...
def show_mypage():
    ...
    ####################################################
        playerNames = []
        for player in list(db.user.find({"time":registeredTime,"type":registeredType},{"_id":False,"username":True,"comment":True})):
            playerNames.append(player['username'])


        # 같은 운동 및 시간 신청한 사람들의 이름과 코멘트 전달      
        # Client에서 players로 받던 데이타 for문으로 변경 필요함
        players = list(db.user.find({"time":registeredTime,"type":registeredType},{"_id":False,"username":True,"comment":True}))
