...
# def insert_all():
#     # db.junglers.drop()  # mystar 콜렉션을 모두 지워줍니다.
#     # names = ['홍길동','김다미','이주형']
#     # for name in names:
...
#     ### 실행하기
# #insert_all()
# db.user.insert_one({"time":1,"type":"a"})
# db.user.insert_one({"time":2,"type":"b"})

A = list(db.user.find({"time" : {'$exists':True}}))
B = list(db.user.find({"_id" : {'$exists':True}}))
print("A", A)
print("B", B)
